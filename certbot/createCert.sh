...
if [ ${production} -eq 1 ]; then
	echo "PROD"
	certbot certonly --config /etc/letsencrypt/letsencrypt.conf --agree-tos -n --expand
else
	echo "TEST"
	certbot certonly --config /etc/letsencrypt/letsencrypt.conf --agree-tos --test-cert -n --expand
fi
killall nginx
